...
export function ProjectsClient({ rows }: Props) {
  ...
  const statusDotColor: Record<string, string> = {
    active: 'bg-green-400',
    quoting: 'bg-purple-400',
    'waiting-approval': 'bg-blue-400',
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 border border-gray-200 bg-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-52"
          />
        </div>
          ...
          {STATUSES.map(s => {
            ...
            return (
              ...
              >
                ...
                {STATUS_LABELS[s]} <span className={on ? 'opacity-70' : 'text-gray-400'}>({count})</span>
              </button>
            )
          })}
        </div>

        {(activeStatuses.size > 0 || search) && (
          <button
            onClick={() => { setActiveStatuses(new Set()); setSearch('') }}
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-5 py-14 text-center text-gray-400 text-sm">
                    ...
                    <button onClick={() => { setActiveStatuses(new Set()); setSearch('') }} className="text-blue-600 hover:underline">Clear filters</button>
                  </td>
                </tr>
              ) : filtered.map(p => (
