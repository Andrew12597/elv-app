...
export function SettingsClient({ employees: initial }: Props) {
  // Own the list in local state — no server refresh needed
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setList(l => [...l, data as Employee].sort((a, b) => a.name.localeCompare(b.name)))
    setForm(empty)
    setAdding(false)
  }
  ...
  async function handleUpdate(id: string) {
    ...
    setSaving(false)
    if (err) { setError(err.message); return }
    setList(l => l.map(e => e.id === id ? { ...e, ...updates } : e))
    setEditingId(null)
  }
  ...
  async function toggleActive(emp: Employee) {
    const { error: err } = await createClient()
      ...
      .eq('id', emp.id)
    if (!err) setList(l => l.map(e => e.id === emp.id ? { ...e, active: !e.active } : e))
  }
          ...
          <form onSubmit={handleAdd} className="px-5 py-4 bg-blue-50/40 border-b border-blue-100 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="sm:col-span-2">
                ...
                <input value={form.name} onChange={e => set('name', e.target.value)} className={inp} placeholder="e.g. James Smith" />
              </div>
              <div>
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={saving}
                ...
                {saving ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving…</> : <><Check className="h-4 w-4" /> Save Employee</>}
              </button>
              ...
              <div key={emp.id} className="px-5 py-4">
                {confirmDeleteId === emp.id ? (
                  <div className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3">
                    ...
                    <p className="text-sm text-red-700 flex-1">Delete <strong>{emp.name}</strong>?</p>
                    <button onClick={() => handleDelete(emp.id)} disabled={deletingId === emp.id}
                      className="bg-red-600 text-white text-sm font-semibold px-4 py-1.5 rounded-lg hover:bg-red-700 transition-colors flex items-center gap-1.5">
                      {deletingId === emp.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}
                      {deletingId === emp.id ? 'Deleting…' : 'Yes, delete'}
                    </button>
                    <button onClick={() => setConfirmDeleteId(null)}
                      className="text-sm text-gray-500 px-3 py-1.5 rounded-lg hover:bg-white transition-colors">
                      Cancel
                    </button>
                  </div>
                ) : editingId === emp.id ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-x-3 mt-0.5">
                        <span className="flex items-center gap-1 text-xs text-gray-400"><DollarSign className="h-3 w-3" />{Number(emp.hourly_rate).toFixed(0)}/hr</span>
                        {emp.phone && <a href={`tel:${emp.phone}`} className="flex items-center gap-1 text-xs text-gray-400 hover:text-blue-600"><Phone className="h-3 w-3" />{emp.phone}</a>}
                        {emp.email && <a href={`mailto:${emp.email}`} className="flex items-center gap-1 text-xs text-gray-400 hover:text-blue-600"><Mail className="h-3 w-3" />{emp.email}</a>}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => toggleActive(emp)} className="text-xs text-gray-400 hover:text-gray-700 px-2 py-1.5 rounded-lg hover:bg-gray-100 transition-colors hidden sm:block">
                        {emp.active ? 'Deactivate' : 'Activate'}
                      </button>
                      <button onClick={() => startEdit(emp)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                        <Pencil className="h-4 w-4" />
                      </button>
                      <button onClick={() => { setConfirmDeleteId(emp.id); setEditingId(null) }} className="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
